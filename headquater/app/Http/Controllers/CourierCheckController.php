...
{
    /**
     * Display the courier checker view.
     */
    public function index()
    {
        $sites = Auth::user()->sites()->where('status', 'active')->get();
        return view('courier-checker.index', compact('sites'));
    }
    ...
    public function check(Request $request)
    {
        ...
        // Use an atomic lock to prevent race conditions.
        $lock = Cache::lock('courier_check.' . $phoneNumber, 15);

        try {
            $lock->block(10); // Wait up to 10 seconds for the lock
            ...
            if ($limit > 0 && $site->usage_courier_checks >= $limit) {
                return response()->json(['error' => 'You have reached your monthly limit for courier API checks for ' . $site->domain], 403);
            }

            // This is now the ONLY place this action increments the usage.
            ...
            return response()->json(['error' => 'This phone number is currently being processed. Please try again in a moment.'], 429);
        } finally {
            optional($lock)->release();
        }
    }
    // --- Private helper methods to call Courier APIs (copied from ProxyController) ---
